...
for( i in 2:10 ) {
  file <- paste("/Users/fabiola/Desktop/BraSNAM2018/dataset/results/user-", i, sep="")
...
log10betUsr <- log10(betDataFrameOrdered$variance)

### closeness variance
cloDataFrame <- data.frame(listOfTimesteps[[1]]$Id)
colnames(cloDataFrame) <- c("user")
for( i in 1:9 ) {
  timeStep <- listOfTimesteps[[i]]
...
log10cloUsr <- log10(cloDataFrameOrdered$variance)


### pageRank variance
prDataFrame <- data.frame(listOfTimesteps[[1]]$Id)
colnames(prDataFrame) <- c("user")
for( i in 1:9 ) {
  timeStep <- listOfTimesteps[[i]]
...
log10prUsr <- log10(prDataFrameOrdered$variance)

## plot graph
pdf("histogram-usr.pdf", width=8.5, height=5.5)
par(mfrow=c(2,3))
hist(log10betUsr, xlab="vari\u00E2ncia (log10)", ylab="frequ\u00EAncia", main="Ponte", col="blue2")
hist(log10prUsr, xlab="vari\u00E2ncia (log10)", ylab="frequ\u00EAncia", main="Influ\u00EAncia", col="darkcyan")
hist(log10cloUsr, xlab="vari\u00E2ncia (log10)", ylab="frequ\u00EAncia", main="Versatilidade", col="chocolate3")
